...
The sensitive property encryption is configured using the properties `keySecret`, `autoGenerate` and `algorithm` in the {crd-ref}/nifi.stackable.tech/v1alpha1/nificluster{crd-ref-release}#clusterConfig.sensitiveProperties[`spec.clusterConfig.sensitiveProperties` {external-link-icon}^] configuration section of the NifiCluster resource.
...
Consult the {crd-ref}/nifi.stackable.tech/v1alpha1/nificluster{crd-ref-release}#clusterConfig.sensitiveProperties.algorithm[reference documentation {external-link-icon}^] for a list of supported algorithms.
