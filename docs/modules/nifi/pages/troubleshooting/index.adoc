...
You are very likely accessing a NiFi >= 2.0 stacklet using HTTPS to secure it's WebUI and an Ingress in front of it.
...
In case you are using multiple NiFi nodes und OpenID connect as authentication method, it is important that a client (such as your Browser) always accesses the same NiFi instance.
