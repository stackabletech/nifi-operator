...
You can inspect the connection settings for the NiFi cluster via script or stackablectl.

==== Via kubectl and jq

In order to access the NiFi web interface you can use the following script to extract the `<ip>:<port>` settings.
...
which should return something like this. Just pick one of the `<ip>:<port>` settings representing one NiFi node:
...
Alternatively, when using stackablectl you can use the service command to get a list of all available endpoints:
...
----
 ...
 nifi       simple-nifi  default    https https://172.18.0.5:32595

 zookeeper  simple-zk    default    zk    172.18.0.5:31377
----

Then connect to `https://<ip>:<port>/nifi` and you should see the NiFi web login. After providing the username `admin` and password `admin` you are redirected to the NiFi web interface.
