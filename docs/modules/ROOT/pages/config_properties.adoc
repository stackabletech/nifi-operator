...
The value for the `password` key has to be a the bcrpyted (weight=12) password that will be used for the admin user.
...
==== Anonymous Access
NiFi can be configured to allow anonymous access to the web UI, this is turned of by default, but can be enabled via the parameter `allowAnonymousAccess`.
...
Anything that is not mentioned here can still be configured via configOverrides.