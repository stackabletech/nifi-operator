...
There are three ways to run the NiFi Operator:
...
----
sudo apt-get update
sudo apt-get install stackable-nifi-operator
----
...
[source,bash]
----
sudo yum install stackable-nifi-operator
----
...
[source,bash]
----
docker run -e NIFI_OPERATOR_LOG=trace -e KUBECONFIG=/root/.kube/k3s.yaml --mount type=bind,source="$HOME/.kube/k3s.yaml",target="/root/.kube/k3s.yaml" -it docker.stackable.tech/stackable/nifi-operator:latest
----
