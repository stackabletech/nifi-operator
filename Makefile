...
VERSION := $(shell cargo metadata --format-version 1 | jq -r '.packages[] | select(.name=="stackable-nifi-operator") | .version')
...
docker-build:
	docker build --force-rm --build-arg VERSION=${VERSION} -t "docker.stackable.tech/stackable/nifi-operator:${VERSION}" -f docker/Dockerfile .
...
version:
	yq eval -i '.version = strenv(VERSION) | .appVersion = strenv(VERSION)' /dev/stdin < deploy/helm/nifi-operator/Chart.yaml
...
crds:
	...
	cargo run --bin stackable-nifi-operator -- crd | yq eval '.metadata.annotations["helm.sh/resource-policy"]="keep"' - > deploy/helm/nifi-operator/crds/crds.yaml
